...
public:
	...
	ESTIMATOR(const ros::NodeHandle& n){

	int status = 0;
	status = acados_sim_create();
	...
	m_eRaptor_sub = n.subscribe("/crazyflie/external_position", 5, &ESTIMATOR::eRaptorCallback, this);
	// subscriber for the IMU linear acceleration and angular velocities from acc and gyro
	m_imu_sub = n.subscribe("/crazyflie/imu", 5, &ESTIMATOR::imuCallback, this);
	// subscriber for the IMU stabilizer euler angles
	m_euler_sub = nh.subscribe("/crazyflie/euler_angles", 5, &ESTIMATOR::eulerCallback, this);

	// publisher for the control inputs of acados (motor speeds to be applied)
	m_motvel_pub = n.advertise<crazyflie_controller::PropellerSpeeds>("/crazyflie/acados_motvel",1);
	// publisher for acados z output for the 1st & N shooting node + z from the mocap
	m_acados_position = n.advertise<geometry_msgs::Vector3>("/crazyflie/acados_traj",1);

	// publisher for the current value of the quaternion
	m_cf_quat = n.advertise<geometry_msgs::Quaternion>("/crazyflie/quat",1);
	// publisher for the current value of the linear velocities
	m_cf_lvb = n.advertise<geometry_msgs::Vector3>("/crazyflie/linear_velo",1);
	// publisher for the current value of the angular velocities
	m_cf_avb = n.advertise<geometry_msgs::Vector3>("/crazyflie/angular_velo",1);
	// subscriber for the motors rpm
	m_motors = n.subscribe("/crazyflie/log1", 5, &ESTIMATOR::motorsCallback, this);
